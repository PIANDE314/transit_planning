...
"""
§ 3.1 Evaluating Roads for Transit Viability — feature extraction per Table 2 :contentReference[oaicite:22]{index=22}
"""

# Map highway types → ordinal (0–5) per Table 2
TYPE_MAP = {
...
def extract_road_segments(
    ...
    """
    Extract only the six road types of interest, buffer 500 m, compute area.

    Args:
        G_proj: Projected OSM graph (EPSG:3857).
        buffer_dist: 500 m buffer radius.

    Returns:
        GeoDataFrame with segment_id, u, v, length_m, highway, geometry, buffer, area_km2.
    """
    rows = []
    ...
    segs["buffer"] = segs.geometry.buffer(buffer_dist)
...
def compute_segment_features(
    ...
    """
    Compute all Table 2 features within 500 m:
      • pop_density, poi_density, wealth_mean, road_density, type_ord
      • MP, OP, EP, Night, Weekend ping densities
      • conn_weekday, conn_weekend ping densities
      • transit_wp, transit_wp_conn densities
    :contentReference[oaicite:23]{index=23}

    Returns:
        (segs enriched, X_scaled, scaler)
    """
    ...
    # 2) POI density
    poi_j = gpd.sjoin(pois_gdf, segs.set_geometry("buffer"),
                      predicate="within", how="inner")
    counts = poi_j.groupby("segment_id").size().reindex(segs.segment_id, 0)
    segs["poi_density"] = counts.values / segs["area_km2"]

    # 3) Wealth mean
    wj = gpd.sjoin(wealth_gdf, segs.set_geometry("buffer"),
                   predicate="within", how="inner")
    ...
    # 6) Ping densities (MP, OP, EP, Night, Weekend)
    def bucket(ts):
        h = ts.hour
        if 7<=h<10: return "MP"
    ...
    for b in ["MP","OP","EP","Night","Weekend"]:
        ...
        c = j.groupby('segment_id').size().reindex(segs.segment_id,0)
        segs[f"{b}_dens"] = c.values / segs["area_km2"]

    # 7) Connected pings (weekday/weekend)
    bufs = list(segs.buffer)
    ids = list(segs.segment_id)
    tree_b = STRtree(bufs)
    wday = p[~p.is_weekend]; wend = p[p.is_weekend]
    t_wday = STRtree(wday.geometry); t_wend = STRtree(wend.geometry)
    cw, cn = [], []
    for sid, buf in zip(ids, bufs):
        # neighbors
        nbrs = [ids[k] for g in tree_b.query(buf)
                for k, gg in enumerate(bufs) if id(gg)==id(g) and gg.intersects(buf) and ids[k]!=sid]
    ...
    for sid, buf in zip(ids, bufs):
        ...
        # connections
        nbr_bufs = [bufs[ids.index(n)] for n in nbrs]
